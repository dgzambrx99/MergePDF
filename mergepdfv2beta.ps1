# This is the in process version that does not delete files nor merge files
...
# v2 creates and sends an email of pdfs in the folders - DZ
...
Param (
	...
	[string]$To = "david@astro.local",
	[string]$Subject = "123 Invoice File List"
	)

# Starts the logfile - DZ
Start-Transcript -path 'C:\Users\administrator.ASTRO\Documents\InvoiceProcessor\logfilebeta.log' -append
...
Get-ChildItem -Exclude $Exclude -Path $Path | Sort-Object name | ?{$_.PSIsContainer -and $_.GetFiles("*.pdf").Count} | Select-Object FullName | % {
	Write-Host "`r`nFullPath: " $_.FullName
	$filepath = $_.FullName
...
{	$SMTPBody = "`r`nFiles Excluded:`r`n$Exclude <br>`n<p>`nThe following files have recently been added/changed:<br>`n<p>`n"
    ...
    Write-Output("`r`nSending E-mail to:`r`n$To")
}
